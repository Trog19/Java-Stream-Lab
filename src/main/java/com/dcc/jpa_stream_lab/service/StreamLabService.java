...
public class StreamLabService {
...

    {
        ...
        Date date1 = new GregorianCalendar(2016, Calendar.JANUARY, 1).getTime();
        return users.findAll().stream().filter(p ->p.getRegistrationDate().before(date1)).toList();
    ...
    public List<User> ProblemSix() {
        ...
        // Return the list
        Date date1 = new GregorianCalendar(2016, Calendar.DECEMBER, 31).getTime();
        Date date2 = new GregorianCalendar(2018, Calendar.JANUARY, 1).getTime();
        return users.findAll().stream().filter(p -> p.getRegistrationDate().before(date2) && p.getRegistrationDate().after(date1)).toList();
    }
    ...
    public List<Product> ProblemEight()
    {
        ...
        User afton = users.findAll().stream().filter(p ->p.getEmail().equals("afton@gmail.com")).findFirst().orElse(null);
        List<ShoppingcartItem> aftonsItems = afton.getShoppingcartItems();
    ...
    public long ProblemNine()
    {
        ...
        List<ShoppingcartItem> odasItems = oda.getShoppingcartItems();
        List<Product> odasProducts = odasItems.stream().map(p ->p.getProduct()).toList();
        List<Integer> odasPrice = odasProducts.stream().map(p -> p.getPrice()).toList();
        int sum = odasPrice.stream().mapToInt(p->p).sum();
...
//    	// Return the list
        ...
        List<ShoppingcartItem> employeesItems = employees.stream().map(p ->p.getShoppingcartItems()).flatMap(s ->s.stream()).collect(Collectors.toList());
        List<Product> employProduct =employeesItems.stream().map(a -> a.getProduct()).toList();

        return employProduct;
    ...
    public Product ProblemTwelve()
    {
        ...
        newProduct.setDescription("5wt Fly Fishing Rod");
        newProduct.setName("Orvis Recon");
        newProduct.setPrice(799);
    ...
    public ShoppingcartItem ProblemFourteen()
    {
        ...
        Product davesProduct = products.findAll().stream().filter(s ->s.getName().equals("Orvis Recon")).findFirst().orElse(null);
        ShoppingcartItem newItem = new ShoppingcartItem();
        newItem.setQuantity(1);
        newItem.setProduct(davesProduct);
        User david = users.findAll().stream().filter(u -> u.getEmail().equals("david@gmail.com")).findFirst().orElse(null);
        david.addShoppingcartItem(newItem);
    ...
    public Product ProblemSixteen()
    {
        ...
        Product orvis = products.findAll().stream().filter(u ->u.getName().equals("Orvis Recon")).findFirst().orElse(null);
        orvis.setPrice(1200);
    	return orvis;
    }
    ...
    public User ProblemSeventeen()
    {
        ...
        Role employeeRole = roles.findAll().stream().filter(r -> r.getName().equals("Employee")).findFirst().orElse(null);
        User employeeDavid = users.findAll().stream().filter(u -> u.getEmail().equals("mike@gmail.com")).findFirst().orElse(null);
        david.addRole(employeeRole);
        return employeeDavid;
//
    }
